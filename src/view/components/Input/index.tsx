...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    { className, error, id, name, label, labelClassName, suffix, ...props },
    ref,
  ) => {
      ...
      <div className="py-2">
        <div className="relative">
          ...
          {label && (
            <label
              className={cn(
                'absolute top-0 left-0 -translate-y-6',
                'text-sm font-medium transition-all duration-300 peer-disabled:cursor-not-allowed peer-disabled:opacity-70 peer-disabled:text-gray-600',
                error && 'text-red-900',
                labelClassName,
              )}
              htmlFor={inputId}
